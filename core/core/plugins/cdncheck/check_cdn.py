...
class CheckCDN:
    ...
    def checkASN(self, ip):
        try:
            ...
            with geoip2.database.Reader(os.path.join(basedir,'GeoLite2-ASN.mmdb')) as reader:
    ...
    def check(self, data):
        if not re.search(r'\d+\.\d+\.\d+\.\d+', data):
            ...
            if data.startswith("http://"):
                _data = data.lstrip("http://")
            elif data.startswith("https://"):
                _data = data.lstrip("https://")
            ip = self.getIP(_data)
        else:
            ip = data
        if ip is None:
        ...
        cdnip = self.checkIP(ip)

        if cdnip == True:
            self.queue.put({"target": data, "cdn": "true",
                           "ip": ip, "detection_method": "ip"})
            return

        cdnasn = self.checkASN(ip)
        if cdnasn == True:
            self.queue.put({"target": data, "cdn": "true",
                           "ip": ip, "detection_method": "asn"})
            return

        if not re.search(r'\d+\.\d+\.\d+\.\d+', data):
            cnames = self.getCNAMES(data)
            match = False
            for i in cnames:
                match = self.matched(i, all_CNAME)
                if match != False:
                    break
            if match != False:
                self.queue.put({"target": data, "cdn": match,
                               "ip": ip, "detection_method": "cname"})
                return
        self.queue.put({"target": data, "cdn": "no",
                       "ip": ip, "detection_method": "none"})
        return
    ...
    def run_async(self):
        with ThreadPoolExecutor(max_workers=100) as pool:
            all_task = [pool.submit(self.check, i) for i in self.targets]
...
if __name__ == '__main__':
    import json
    if len(sys.argv) < 2:
        print("error python3 checkcdn.py www.xx.com www.xxx.com")
        exit()
    dataList = sys.argv[1:]
    cd = CheckCDN(dataList)
    # cd.run()
    cd.run_async()
    result = []
    while not cd.queue.empty():
        result.append(cd.queue.get())
    result_json = json.dumps(
        result, indent=4, sort_keys=True, ensure_ascii=False)
    print(result_json)

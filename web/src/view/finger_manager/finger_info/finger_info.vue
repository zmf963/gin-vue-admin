...
const getTableData = async () => {
  const table = await getFingerInfoList({
    page: page.value,
...
const updateFingerInfoFunc = async (row) => {
  ...
  if (res.code === 0) {
    ...
    formData.value.content = JSON.stringify(res.data.refinger_info.content)
...
const enterDialog = async () => {
  ...
  if (typeof formData.value.link_vul === "string") {
    formData.value.link_vul = formData.value.link_vul.split(",");
  }
  formData.value.content = JSON.parse(formData.value.content);
  switch (type.value) {
    case "create":

  ...
  <div>
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
            ...
            <el-form-item label="关联漏洞">
              <el-input v-model="searchInfo.link_vul" placeholder="搜索条件" />
            </el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
            ...
            <el-table-column align="left" label="指纹详情" prop="content" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateFingerInfoFunc(scope.row)">编辑</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" label-width="80px">
        ...
        <el-form-item label="关联漏洞:">
            ...
            <el-input v-model.number="formData.link_vul" clearable placeholder="请选择" />
...
import {
  createFingerInfo,
  deleteFingerInfo,
  deleteFingerInfoByIds,
  updateFingerInfo,
  findFingerInfo,
  getFingerInfoList
} from '@/api/finger_info'
...
const getTableData = async() => {
  const table = await getFingerInfoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteFingerInfoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateFingerInfoFunc = async(row) => {
    const res = await findFingerInfo({ _id: row._id })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.refinger_info
...
// 删除行
const deleteFingerInfoFunc = async (row) => {
    const res = await deleteFingerInfo({ _id: row._id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
      ...
      switch (type.value) {
        case 'create':
          res = await createFingerInfo(formData.value)
          break
        case 'update':
          res = await updateFingerInfo(formData.value)
          break
        default:
          res = await createFingerInfo(formData.value)
          break

  ...
  <div>
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
            ...
            <el-form-item label="开发者">
              <el-input v-model="searchInfo.developer" placeholder="搜索条件" />
            </el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
            ...
            <el-table-column align="left" label="开发者" prop="developer" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateAppInfoFunc(scope.row)">编辑</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" label-width="80px">
        ...
        <el-form-item label="开发者:">
            ...
            <el-input v-model="formData.developer" clearable placeholder="请输入" />
...
const getTableData = async() => {
  const table = await getAppInfoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteAppInfoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateAppInfoFunc = async(row) => {
    const res = await findAppInfo({ _id: row._id })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.reapp_info
...
// 删除行
const deleteAppInfoFunc = async (row) => {
    const res = await deleteAppInfo({ _id: row._id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
      ...
      switch (type.value) {
        case 'create':
          res = await createAppInfo(formData.value)
          break
        case 'update':
          res = await updateAppInfo(formData.value)
          break
        default:
          res = await createAppInfo(formData.value)
          break

  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateDocInfoFunc(scope.row)">编辑</el-button>
...
const getTableData = async() => {
  const table = await getDocInfoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteDocInfoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateDocInfoFunc = async(row) => {
    const res = await findDocInfo({ _id: row._id })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.redoc_info
...
// 删除行
const deleteDocInfoFunc = async (row) => {
    const res = await deleteDocInfo({ _id: row._id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
      ...
      switch (type.value) {
        case 'create':
          res = await createDocInfo(formData.value)
          break
        case 'update':
          res = await updateDocInfo(formData.value)
          break
        default:
          res = await createDocInfo(formData.value)
          break

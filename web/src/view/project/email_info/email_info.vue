  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateEmailInfoFunc(scope.row)">编辑</el-button>
...
const getTableData = async() => {
  const table = await getEmailInfoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteEmailInfoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateEmailInfoFunc = async(row) => {
    const res = await findEmailInfo({ _id: row._id })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.reemail_info
...
// 删除行
const deleteEmailInfoFunc = async (row) => {
    const res = await deleteEmailInfo({ _id: row._id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
      ...
      switch (type.value) {
        case 'create':
          res = await createEmailInfo(formData.value)
          break
        case 'update':
          res = await updateEmailInfo(formData.value)
          break
        default:
          res = await createEmailInfo(formData.value)
          break

  ...
  <div>
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
            ...
            <el-form-item label="密码类型">
              <el-input v-model="searchInfo.password_type" placeholder="搜索条件" />
            </el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
            ...
            <el-table-column align="left" label="密码类型" prop="password_type" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateKeysFunc(scope.row)">编辑</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" label-width="80px">
        ...
        <el-form-item label="密码类型:">
            ...
            <el-input v-model="formData.password_type" clearable placeholder="请输入" />
...
const getTableData = async() => {
  const table = await getKeysList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteKeysByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateKeysFunc = async(row) => {
    const res = await findKeys({ _id: row._id })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.rekeys
...
// 删除行
const deleteKeysFunc = async (row) => {
    const res = await deleteKeys({ _id: row._id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
      ...
      switch (type.value) {
        case 'create':
          res = await createKeys(formData.value)
          break
        case 'update':
          res = await updateKeys(formData.value)
          break
        default:
          res = await createKeys(formData.value)
          break

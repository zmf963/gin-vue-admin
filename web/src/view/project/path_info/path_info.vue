  ...
  <div>
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
            ...
            <el-form-item label="hostinfo">
              <el-input v-model="searchInfo.hostinfo" placeholder="搜索条件" />
            </el-form-item>
            ...
            <el-form-item label="response_size">
              <el-input v-model="searchInfo.response_size" placeholder="搜索条件" />
            </el-form-item>
          
        
          
            <el-form-item label="port_id">
              <el-input v-model="searchInfo.port_id" placeholder="搜索条件" />
            </el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
            ...
            <el-table-column align="left" label="path" prop="path" width="120" />
            ...
            <el-table-column align="left" label="response_size" prop="response_size" width="120" />
          
        
          
            <el-table-column align="left" label="port_id" prop="port_id" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updatePathInfoFunc(scope.row)">编辑</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" label-width="80px">
        ...
        <el-form-item label="port_id:">
            ...
            <el-input v-model="formData.port_id" clearable placeholder="请输入" />
...
const getTableData = async() => {
  const table = await getPathInfoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deletePathInfoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updatePathInfoFunc = async(row) => {
    const res = await findPathInfo({ _id: row._id })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.repath_info
...
// 删除行
const deletePathInfoFunc = async (row) => {
    const res = await deletePathInfo({ _id: row._id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
      ...
      switch (type.value) {
        case 'create':
          res = await createPathInfo(formData.value)
          break
        case 'update':
          res = await updatePathInfo(formData.value)
          break
        default:
          res = await createPathInfo(formData.value)
          break

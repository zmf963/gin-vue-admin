  ...
  <div>
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline">
        <el-form-item label="文档名称">
          <el-input v-model="searchInfo.doc_name" placeholder="搜索条件" />
        </el-form-item>
    ...
    <div class="gva-table-box">
      ...
      <el-table ref="multipleTable" style="width: 100%" tooltip-effect="dark" :data="tableData" row-key="ID"
        ...
        <el-table-column align="left" label="文档名称" prop="doc_name" width="120" />
        <el-table-column align="left" label="链接" prop="link" width="120" />
        <el-table-column align="left" label="来源" prop="source" width="120" />
        <el-table-column align="left" label="项目ID" prop="project_id" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateProjectDocFunc(scope.row)">
              编辑</el-button>
      ...
      <div class="gva-pagination">
        <el-pagination layout="total, sizes, prev, pager, next, jumper" :current-page="page" :page-size="pageSize"
          :page-sizes="[10, 3, 5, 15, 30, 50, 100]" :total="total" @current-change="handleCurrentChange"
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" label-width="80px">
        ...
        <el-form-item label="文档名称:">
          <el-input v-model="formData.doc_name" clearable placeholder="请输入" />
        </el-form-item>
...
import {
  createProjectDoc,
  deleteProjectDoc,
  deleteProjectDocByIds,
  updateProjectDoc,
  findProjectDoc,
  getProjectDocList
} from '@/api/project_doc'
...
const getTableData = async () => {
  const table = await getProjectDocList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async () => {
  ...
  const res = await deleteProjectDocByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updateProjectDocFunc = async (row) => {
  const res = await findProjectDoc({ _id: row._id })
  type.value = 'update'
  if (res.code === 0) {
    formData.value = res.data.reproject_doc
    dialogFormVisible.value = true
  }
}

// 删除行
const deleteProjectDocFunc = async (row) => {
  const res = await deleteProjectDoc({ _id: row._id })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async () => {
  ...
  switch (type.value) {
    case 'create':
      res = await createProjectDoc(formData.value)
      break
    case 'update':
      res = await updateProjectDoc(formData.value)
      break
    default:
      res = await createProjectDoc(formData.value)
      break
